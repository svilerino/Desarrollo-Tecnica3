...
public class MiFrame extends JFrame implements ActionListener{
	...
	private JCheckBox jchk_Periodo;
	private String modelos[]={"Java", "Trigonometrico", "CenteredSquare - Numeric", "CenteredSquare - String", "Congruencial", "Caos"};
	...
	public MiFrame(){
		...
		//Panel graficos
		pnl_graficos=new pnlGrafico(new AnalisisAnaliticoArray(0, false));
		...
		jchk_Periodo=new JCheckBox("Buscar Periodo");
		jchk_Periodo.setBackground(Color.BLUE);
		jchk_Periodo.setBounds(20, 170, 150, 20);
		jchk_Periodo.addActionListener(this);
		...
		pnl_controles.add(jchk_Periodo);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==jcmb_Modelo){
			pnl_graficos=new pnlGrafico(new AnalisisAnaliticoArray(jcmb_Modelo.getSelectedIndex(),jchk_Periodo.isSelected()));
			pnl_graficos.setLayout(null);
			pnl_graficos.setBounds(PSGRA_X, PSGRA_Y, GRA_X, GRA_Y);
			pnl_graficos.setVisible(false);
		}

		if(e.getSource()==jchk_Periodo){
			if((jcmb_Modelo.getSelectedIndex()==1)||(jcmb_Modelo.getSelectedIndex()==5)){
				System.out.println("Sen - Caos");
				jchk_Periodo.setSelected(true);
			}else{
				System.out.println("Modelo invalido para buscar Periodo.");
				jchk_Periodo.setSelected(false);
			}
		}
